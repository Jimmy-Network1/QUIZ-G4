...
export default function TournamentBracketScreen(): JSX.Element {
  ...
  if (!tournament) return <View style={styles.container}><Text>Loading Bracket...</Text></View>;
  ...
  return (
    <ImageBackground source={LobbyBg} style={styles.background}>
      <LinearGradient colors={['rgba(0,0,0,0.8)', 'rgba(0,0,0,0.5)']} style={styles.gradient}>
            ...
            <View key={rIdx} style={styles.roundColumn}>
              ...
              <ScrollView>
                {round.matches.map((match: any, mIdx: number) => {
                  const isMyMatch = match.player1?._id === userId || match.player2?._id === userId;
                  const canJoin = isMyMatch && match.status === 'pending';

                  return (
                    <View key={mIdx} style={[styles.matchCard, isMyMatch && styles.myMatchCard]}>
                      <View style={styles.playerRow}>
                        <Text style={[styles.playerName, match.winner === match.player1?._id && styles.winner]}>
                      ...
                      <View style={styles.playerRow}>
                        <Text style={[styles.playerName, match.winner === match.player2?._id && styles.winner]}>
        ...
        <TouchableOpacity style={styles.backButton} onPress={() => navigation.navigate(AuthenticatedScreens.MainMenuScreen)}>
...
const styles = StyleSheet.create({
  ...
  container: {flex: 1, backgroundColor: colorList.darkBackgroundBlue, justifyContent: 'center', alignItems: 'center'},
  ...
  roundTitle: {color: '#fff', fontWeight: 'bold', fontSize: 18, marginBottom: 20, backgroundColor: 'rgba(255,255,255,0.1)', paddingHorizontal: 20, paddingVertical: 5, borderRadius: 20},
  matchCard: {
    width: '100%',
  ...
  winner: {color: colorList.vibrantCyan, fontWeight: 'bold', textShadowColor: colorList.vibrantCyan, textShadowRadius: 5},
